...
IMG_MEAN = np.array((104.00698793,122.67891434,116.66876762), dtype=np.float32)
global_step = 0
class Model(object):
	...
	def train(self):
		...
		if self.conf.fine_tune:
			ckpt = tf.train.get_checkpoint_state(self.conf.modeldir)  # 训练断点回复
			if ckpt and ckpt.model_checkpoint_path:  # 如果存在checkpoint文件 则恢复sess
				self.saver.restore(self.sess, ckpt.model_checkpoint_path[-1])
				global_step = int(ckpt.all_model_checkpoint_path[-1].split('.')[-1].split("-")[-1])
				print("step:%d Model restored..."%global_step)

		# Start queue threads.
		threads = tf.train.start_queue_runners(coord=self.coord, sess=self.sess)

		# Train!
		for step in range(self.conf.num_steps+1 - global_step):

			start_time = time.time()
			feed_dict = { self.curr_step : step + global_step }

			if (step + global_step) % self.conf.save_interval == 0:
				##每1000步可添加查看精准度的函数
				loss_value, images, labels, preds, summary, _ = self.sess.run(
					...
					self.train_op],
					feed_dict=feed_dict)
				self.summary_writer.add_summary(summary, step + global_step)
				self.save(self.saver, step + global_step)
			else:
				loss_value, _ = self.sess.run([self.reduced_loss, self.train_op],
					feed_dict=feed_dict)
			##w
			duration = time.time() - start_time
			print('step {:d} \t loss = {:.3f}, ({:.3f} sec/step)'.format(step + global_step, loss_value, duration))
			write_log('{:d}, {:.3f}'.format(step + global_step, loss_value), self.conf.logfile)
	...
	def predict(self):
		...
		for step in range(self.conf.test_num_steps):
			...
			img_name = image_list[step].split('/')[1].split('.')[0]
	...
	def test_setup(self):
		...
		with tf.name_scope("create_inputs"):
			reader = ImageReader(
				...
				IMG_MEAN,
				self.coord)
			image, label = tf.to_float(reader.image) , tf.to_float(reader.label )# [h, w, 3 or 1]
		...
		weights = tf.cast(tf.to_int32(temp), tf.int32)

		# fix for tf 1.3.0
		gt = tf.where(temp, gt, tf.cast(temp, tf.float32))
